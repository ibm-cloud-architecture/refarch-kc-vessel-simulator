...
A fleet will have one to many vessels. Fleet has id and name. vessel has ID, name, status, position, port and type. vessel carries containers. Container has id, and metrics like amp, temperature. Here is an example of JSON document illustrating this model:
```json
 {
    "id": "f1",
    ...
    "vessels": [
      {
        ...
        "maxColumn": 7,
         ...
         "containers": [
             {"id":"c_2","type":"Reefer","temperature":10,"amp":46,"status":"RUN","row":0,"column":2,"vesselId":"MarieRose"}
         ],
      }],
}
```
...
- [x] Support simulate vessel movement and refrigerated containers metrics simulation via  REST api to be easily consumable from backend for frontend component using a POST verb.
...
- [ ] Generate vessel position event x seconds, to demonstrate vessel movement representing x minutes of real time. Like in a video game.
- [x] Generate, at each position update, the n container metric events for all container carried in the moving vessel
...
* `ibm.labs.kc.dao` data access object for vessel and fleet. Use mockup no backend DB yet.
* `ibm.labs.kc.event.model` event definitions for the kafka topic payload
* `ibm.labs.kc.simulator` simulators for the demo as we do not have real vessels... yet.
...
As an example of TDD applied to this project, we want to test the "get the list of fleets" feature. As this code is built by iteration, the first iteration is to get the fleet definition and vessels definition from files. The `src/main/resources` folder includes a json file to define the fleets. We do not need an external datasource for this mockup solution.
...
The simulation of the different container events is done in the class `BadEventSimulator`. But this class is used in a Runner, the `vesselRunner`. The approach is to move the vessel to the next position as defined in the separate csv file (named by the vessel's name), then to send the new vessel position, and the container metrics at that position as events. So the simulator uses two Kafka producers, one for the vessel position and one for the container metrics.
...
@Test
	public void validateContainerDown() {
        serv =  new vesselService();
		vesselSimulationControl ctl = new vesselSimulationControl("JimminyCricket", vesselSimulationControl.REEFER_DOWN);
		ctl.setNumberOfMinutes(1);
		Response res = serv.performSimulation(ctl);
        vessel s = (vessel)res.getEntity();
        Assert.assertTrue(s.getContainers().get(0).get(3).getStatus().equals(Container.STATUS_DOWN));
    }
```
Event after adding the vesselSimulationControl Java Bean and the operation performSimulation into the service... we have a problem... How to unit tests without sending message to Kafka?.

The vesselRunner is a Runnable class and uses the `positionPublisher` and `containerPublisher` which are standard Kafka producers.
Here is a code snippet for the `run()` method of the `vesselRunner`: The vessel positions are loaded from a file in the class loader and then for each container in the boat, send metrics.

```java
try  {
    for (Position p : this.positions) {
        // vessels publish their position to a queue
        vesselPosition sp = new vesselPosition(this.vesselName,p.getLatitude(),p.getLongitude());
        positionPublisher.publishvesselPosition(sp);

        // Then publish the state of their containers
        for (List<Container> row :  vessel.getContainers()) {
            for (Container c : row) {
                ContainerMetric cm = BadEventSimulator.buildContainerMetric(this.vesselName,c,dateFormat.format(currentWorldTime));
...
Add a constructor in `vesselRunner` so we can inject the producer. The test can use mockup at the simulator level or at the producer level. Here is an example in the unit test class of injecting for producer:
```java
 @Mock
 ...
 public void validateContainerFire() {
    ...
    vesselRunner sr = new vesselRunner(positionPublisherMock, containerPublisherMock);
	vesselSimulator s = new vesselSimulator(sr);
    serv =  new vesselService(DAOFactory.buildOrGetvesselDAOInstance("Fleet.json"),s);
    // ..
    Response res = serv.performSimulation(ctl);
}
```
...
```java
@Operation(summary = "Get fleet by fleet name",description=" Retrieve a fleet with vessels from is unique name")
@APIResponses(
    value = {
        @APIResponse(
            responseCode = "404",
	...
	public Fleet getFleetByName(
			@Parameter(
		            description = "The fleetname to get vessels data",
		            required = true,
...
A summary of the operations defined for this simulator are:
 ...
 | GET '/fleetms/fleets/:fleetname' | Get the vessels of a given fleet |
 | POST '/fleetms/fleets/simulate' | Start to simulate vessels movements |
 | POST '/fleetms/vessels/simulate' | Start to simulate vessel movements and container metrics generation |
...
One interesting integration test is defined in the class `it.FireContainerSimulationIT.java` as it starts a Thread running a ContainerConsumer (bullet 1 in figure below) which uses Kafka api to get `Container events` (class `ibm.labs.kc.event.model.ContainerMetric`) from the `bluewaterContainer` topic, and then calls the POST HTTP end point (2): `http://localhost:9080/fleetms/vessels/simulate` with a simulator control object (`ibm.labs.kc.dto.model.vesselSimulationControl`). The application is producing vessel position events and container metrics events at each time slot (3). The consumer is getting multiple events (4) from the topic showing some containers are burning:

```json
{"id":"c_2","type":"Reefer","temperature":150,"amp":46,"status":"FIRE","row":0,"column":2,"vesselId":"JimminyCricket"},
{"id":"c_3","type":"Reefer","temperature":150,"amp":42,"status":"FIRE","row":0,"column":3,"vesselId":"JimminyCricket"}
```
